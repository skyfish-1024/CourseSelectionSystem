...
func GetChoseCourses(StuNum string) ([]string, int, error) {
	Courses, err := db.Rdb.GetStuCourse(StuNum)
	if err != nil && err != gorm.ErrRecordNotFound {
		return nil, 0, err
	}
...
func OpenCourse(CourseNum string) error {
	...
	_, err = db.Rdb.OpenCourse(CourseNum, LeftStu)
	if err != nil {
		return err
	}
	return nil
...
func CloseCourse(CourseNum string) ([]string, error) {
	...
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return errStu, errors.New("课程不存在")
		}
		return errStu, err
	}
	if data.State == 2 {
	...
	students, err1 := db.Rdb.CloseCourse(CourseNum)
	if err != nil {
		return errStu, err1
	}
...
func CourseChoose(CourseNum string, StuNUm string) error {
	err := db.Rdb.CourseAddStu(CourseNum, StuNUm)
	if err != nil {
		return err
	}
	return nil
...
func CourseDrop(CourseNum string, StuNUm string) error {
	_, err := db.Rdb.CourseDelStu(CourseNum, StuNUm)
	if err != nil {
		return err
	}
	return nil
